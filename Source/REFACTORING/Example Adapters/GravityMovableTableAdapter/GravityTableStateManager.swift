...
//import Foundation
//
//open class GravityTableStateManager: AbstractStateManager {
//
//    // MARK: - Properties
//
//    weak var adapter: GravityMovableTableAdapter?
//
...
//            let tableView = self.adapter?.collection
//        else {
//            return
//        }
//
...
//        adapter?.collection.reloadRows(at: indexPaths, with: .none)
//    }
//
...
//        guard let tableView = self.adapter?.collection else { return }
//
...
//        guard let index = self.findGenerator(oldGenerator), let table = self.adapter?.collection else { return }
//
//        table.beginUpdates()
...
//        self.adapter?.collection.reloadSections(IndexSet(arrayLiteral: indexOfHeader), with: animation)
//    }
//
...
//        }
//
//        adapter?.collection.insertRows(at: indexPaths, with: .none)
//    }
//
...
//                         needScrollAt scrollPosition: UITableView.ScrollPosition? = nil,
//                         needRemoveEmptySection: Bool = false) {
//        guard let table = self.adapter?.collection else { return }
//
//        // perform update
//        table.beginUpdates()
//        self.generators[index.sectionIndex].remove(at: index.generatorIndex)
